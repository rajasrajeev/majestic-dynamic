	...
	public function get_products() {
	  ...
	  public function addproducts_post() {
		...
		if (!$this->upload->do_upload('image')) {
			...
			$this->load->view('admin/products/create', $error);
		} else {
			$data = $this->upload->data();
			$sport_data = array(
				'title' => $this->input->post('title'),
				'content' => $this->input->post('content'),
				'image' => "uploads/products/".$data['file_name']
			);

			// Save the sport data to the database
			$this->db->insert('products', $sport_data);
	  ...
	  public function updateproduct_post() {
		...
		if ($this->upload->do_upload('image')) {
			...
			$data = $this->upload->data();
			$service_data['image'] = "uploads/products/" . $data['file_name'];
		} else {
			...
			if ($this->upload->display_errors() !== '<p>You did not select a file to upload.</p>') {
				...
				$error = array('error' => $this->upload->display_errors());
				$this->load->view('admin/products/edit', $error);
				return;
			}
		...
		$this->db->update('products', $service_data);
